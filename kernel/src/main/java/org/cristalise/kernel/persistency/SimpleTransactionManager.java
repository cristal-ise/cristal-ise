...
/**
 * A simple implementation of transactional behavior which works on none transactional backends
 * (e.g. XMLClusterStorage). It uses a local cache to store and a transaction key (aka locker)
...
public class SimpleTransactionManager implements TransactionManager {
            ...
            for (TransactionEntry thisEntry : pendingTransactions.get(locker)) {
                if (itemPath.equals(thisEntry.itemPath) && thisEntry.path.startsWith(path)) {
                    if (thisEntry.obj == null)
                        throw new PersistencyException("Cluster " + path + " has been deleted in " + itemPath +
     ...
     * @throws PersistencyException 
     */
    private void createTransactionEntry(ItemPath itemPath, C2KLocalObject obj, String  path, ArrayList<TransactionEntry> lockerTransaction) 
            throws PersistencyException
    {
    ...
    public void commit(Object locker) throws PersistencyException {
        synchronized(locks) {
            ...
            try {
                ...
                storage.abort(locker);
                log.error("commit() - Database may be in an inconsistent state.");
                dumpPendingTransactions(0);
                AbstractMain.shutdown(1);
            }
        }
