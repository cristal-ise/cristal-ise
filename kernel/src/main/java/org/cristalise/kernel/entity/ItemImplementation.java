...
public class ItemImplementation implements ItemOperations {
            ...
            throws AccessRightsException, InvalidDataException, PersistencyException
    {
        ...
        try {
            ...
            Viewpoint initViewpoint = null;
            if (StringUtils.isNotBlank(initViewpointString)) {
                initViewpoint = (Viewpoint)Gateway.getMarshaller().unmarshall(initViewpointString);
            }

            CollectionArrayList colls = null;
            if (StringUtils.isNotBlank(initCollsString) && !initCollsString.equals("<NULL/>")) {
                colls = (CollectionArrayList) Gateway.getMarshaller().unmarshall(initCollsString);
            }

            CompositeActivity domainWf = null;
            if (StringUtils.isNotBlank(initWfString) || !initWfString.equals("<NULL/>")) {
                domainWf = (CompositeActivity) Gateway.getMarshaller().unmarshall(initWfString);
            }
            else {
                // create empty domain workflow
                log.warn("initialise({}) - workflow XML was empty, using 'NoWorkflow' definition", mItemPath);
                domainWf = (CompositeActivity)LocalObjectLoader.getCompActDef("NoWorkflow", 0).instantiate();
            }

            storeNewItem(agentPath, mItemPath, props, domainWf, colls, initViewpoint, initOutcomeString, locker);
        }
        catch (Exception ex) {
            log.error("initialise({}) - Properties:{}", mItemPath, propString, ex);
            ...
            PersistencyException, ObjectAlreadyExistsException, InvalidCollectionModification
    {
        ...
        try {
            ...
            String finalOutcome = lifeCycle.requestAction(agent, delegate, stepPath, mItemPath, transitionID, requestData, attachmentType, attachment, locker);
    ...
    public String queryData(String path) throws AccessRightsException, ObjectNotFoundException, PersistencyException {
        ...
        log.info("queryData(" + mItemPath + ") - " + path);
        ...
        try { // check for cluster contents query
            ...
            log.warn("queryData(" + mItemPath + ") - " + path + " Failed: " + ex.getClass().getName());
            throw new PersistencyException("Server exception: " + ex.getClass().getName());
        }

        log.trace("queryData(" + mItemPath + ") - result:" + result);
