...
public class ElementaryActDescAccess extends ResourceAccess {
    ...
    public Response listAllStateMachines(@CookieParam(COOKIENAME) Cookie authCookie, @Context UriInfo uri) {
        ...
        return listAllResources(ELEM_ACT_DESC_RESOURCE, uri);
    }

    @GET
    @Path("{name}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response listElementaryActDescVersions(@PathParam("name") String name, @CookieParam(COOKIENAME) Cookie authCookie, @Context UriInfo uri) {
        checkAuthCookie(authCookie);
