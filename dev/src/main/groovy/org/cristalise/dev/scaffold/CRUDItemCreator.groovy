...
class CRUDItemCreator extends StandardClient {
    ...
    private void updateRecord(Map record, Schema updateSchema, String itemRoot) {
        ...
        record.each { name, value ->  
            def field = (Field)builder.findChildStructure((String)name)
            assert field

            String referencedItemType = field.getReferencedItemType()

            if (referencedItemType) {
                def uuid = agent.getItem("$itemRoot/$name").getPath().getUUID().toString()
                log.info('updateRecord() - field:{} replacing value {} with {}', name, value, uuid)
                record[name] = uuid
            }
        }
    }
    ...
    public ItemProxy createItemWithConstructorAndCheck(Map record, String factoryPath) {
        ...
        if (dp.exists()) {
            ...
            if (itemName && updateMode == ERASE) agent.execute(item, Erase.class);
        }
