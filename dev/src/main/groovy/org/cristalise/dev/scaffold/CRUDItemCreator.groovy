...
import static org.cristalise.dev.scaffold.CRUDItemCreator.CreateBehaviour.SKIP
import static org.cristalise.dev.scaffold.CRUDItemCreator.CreateBehaviour.UPDATE
...
class CRUDItemCreator extends StandardClient {
    ...
    public enum CreateBehaviour {ERASE, UPDATE, SKIP}

    public CRUDItemCreator() {}

    public CRUDItemCreator(AgentProxy a) {
        agent = a
    }
     ...
     * @return
     */
    public ItemProxy createItemWithConstructorAndCheck(Map record, String factoryPath, CreateBehaviour eraseOrWhat) {
        ...
        if (dp.exists()) {
            ...
            if (itemName && eraseOrWhat == ERASE) agent.execute(item, Erase.class);
        }
        ...
        if (!item) {
            ...
            if (eraseOrWhat != SKIP) updateItemAndCheck(item, itemRoot, itemName, updateSchema, record)
        }
     ...
     * @return
     */
    public ItemProxy createItemWithUpdateAndCheck(Map record, String factoryPath, CreateBehaviour eraseOrWhat) {
        ItemProxy factory = agent.getItem(factoryPath)
        String itemRoot = factory.getProperty('Root')
        String itemName = record.Name ?: ''
        ItemProxy item = null
        ...
        if (!item) item = createItemAndCheck(factory, itemRoot, itemName)
        ...
        if (eraseOrWhat != SKIP) updateItemAndCheck(item, itemRoot, itemName, getUpdateSchema(factory), record)
    ...
    public void createItems(File xlsx, String itemType) {
        TabularGroovyParser parser = TabularGroovyParserBuilder.build(xlsx, itemType, 2)

        parser.eachRow() { Map<String, Object> record, int i ->
            createItemWithUpdateAndCheck(record, 'factoryPath', UPDATE)
        }
    }
    ...
    public static void main(String[] args) {
        ...
        if (!options.c || !options.l || !options.s) {
          log.info("Missing configuration files")
          cli.usage()
          return
        }
        ...
        def creator = new CRUDItemCreator()
