...
15. **Item History & Event Tracking**: Developed the `BasicItemHistory` component to visualize audit logs of an Item. Integrated the backend history endpoint with a lazy-loading strategy, ensuring data is only fetched upon tab activation. Refactored the history display to use a data-driven column configuration and integrated the Transloco i18n system to provide full multi-language support (EN, FR, DE, HU) for all history-related metadata and UI labels.
16. **Admin Routing Migration**: Migrated the primary application layout and navigation route from `/dashboard` to `/admin`. Updated all related links, redirects, services, and unit test suites to use the new `/admin` route path.
