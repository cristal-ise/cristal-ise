...
if (createAgent) {
    ...
    agent.execute(newAgent, SetAgentPassword, params)
}
...
String getInitaliseOutcomeXML(ItemProxy item, Outcome outcome, String itemName) {
    ...
    if (initialiseNode) {
        ...
        throw new InvalidDataException("Script.InstantiateItem - invalid path:/Factory_NewInstanceDetails/SchemaInitialise/$updateSchema")
    }
}
...
String getDomainRoot(ItemProxy item, Job job) {
    ...
    if (!root) throw new InvalidDataException("Script.InstantiateItem - Define property:'Root' for either Activity or for Item")
