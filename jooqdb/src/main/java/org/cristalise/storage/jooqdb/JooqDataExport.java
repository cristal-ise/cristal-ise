...
public class JooqDataExport {
     ...
     * @throws IOException
     */
    public void exportDataByLastSyncDate(DSLContext context, LocalDateTime lastSyncDate,
            LocalDateTime syncDate) throws ObjectNotFoundException, IOException {
     ...
     * @throws IOException
     */
    public void exportDataByEvent(DSLContext context, Schema schema, LocalDateTime lastSyncDate,
            LocalDateTime syncDate) throws IOException {
        ...
        context.selectFrom(EXPORT_TABLE).where(TIMESTAMP.between(lastSyncDate, syncDate))
                .fetchLazy().formatCSV(writer);
    }

    public void exportDataByItem(DSLContext context, Schema schema, LocalDateTime lastSyncDate,
            LocalDateTime syncDate) throws IOException {
        exportDataProcess(context, schema, lastSyncDate, syncDate, ITEM_TABLE);
    }

    public void exportDataByProperty(DSLContext context, Schema schema, LocalDateTime lastSyncDate,
            LocalDateTime syncDate) throws IOException {
        exportDataProcess(context, schema, lastSyncDate, syncDate, PROPERTY_TABLE);
    }

    public void exportDataByLifeCycle(DSLContext context, Schema schema, LocalDateTime lastSyncDate,
            LocalDateTime syncDate) throws IOException {
        exportDataProcess(context, schema, lastSyncDate, syncDate, LIFECYCLE_TABLE);
    }

    public void exportDataByJob(DSLContext context, Schema schema, LocalDateTime lastSyncDate,
            LocalDateTime syncDate) throws IOException {
        exportDataProcess(context, schema, lastSyncDate, syncDate, JOB_TABLE);
    }

    public void exportDataByCollection(DSLContext context, Schema schema,
            LocalDateTime lastSyncDate, LocalDateTime syncDate) throws IOException {
        exportDataProcess(context, schema, lastSyncDate, syncDate, COLLECTION_TABLE);
    }



    public void exportDataByOutcome(DSLContext context, Schema schema, LocalDateTime lastSyncDate,
            LocalDateTime syncDate) throws IOException {
        exportDataProcessWithJoin(context, schema, lastSyncDate, syncDate, OUTCOME_TABLE);
    }

    public void exportDataByViewpoint(DSLContext context, Schema schema, LocalDateTime lastSyncDate,
            LocalDateTime syncDate) throws IOException {
        exportDataProcessWithJoin(context, schema, lastSyncDate, syncDate, VIEWPOINT_TABLE);

    }

    public void exportDataByAttachment(DSLContext context, Schema schema,
            LocalDateTime lastSyncDate, LocalDateTime syncDate) throws IOException {
    ...
    public void exportDataProcessWithJoin(DSLContext context, Schema schema,
            LocalDateTime lastSyncDate, LocalDateTime syncDate, String tableName)
            throws IOException {
        FileStringUtility.createNewDir(EXPORTED_DIR);
        ...
        context.select().from(EXPORT_TABLE).join(EVENT)
                .on(EXPORT_UUID.equal(EVENT_UUID), EXPORT_EVENTID.equal(EVENT_ID))
                .where(TIMESTAMP.between(lastSyncDate, syncDate)).fetchLazy().formatCSV(writer);
    }
    ...
    public void exportData(DSLContext context, String tableName, File file)
     ...
     * @throws IOException
     */
    private void exportDataProcess(DSLContext context, Schema schema, LocalDateTime lastSyncDate,
            LocalDateTime syncDate, String tableName) throws IOException {
        FileWriter writer = new FileWriter(new File(EXPORTED_DIR + "/" + tableName + ".csv"));
        ...
        context.select().from(EXPORT_TABLE)
                .where(EXPORT_UUID.in(context.selectDistinct(EVENT_UUID).from(EVENT)
                        .where(TIMESTAMP.between(lastSyncDate, syncDate))))
                .fetchLazy().formatCSV(writer);
    }

    public Schema getPublicSchema(DSLContext context) {
        return context.meta().getSchemas().stream().filter(s -> s.getName().equals(PUBLIC_SCHEMA))
                .findFirst().get();
    }
}
