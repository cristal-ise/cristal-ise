/**
 * This file is part of the CRISTAL-iSE jOOQ Cluster Storage Module. Copyright (c) 2001-2017 The
...
public class JooqDataImport {
    ...
    public void importOutcomeData(DSLContext context) throws IOException, ObjectNotFoundException {
        ...
        if (!Objects.isNull(schema)) {
            ...
            context.loadInto(OUTCOME_TABLE).onDuplicateKeyUpdate().commitAfter(100)
                    .loadCSV(inputStream).fields(UUID, SCHEMA_NAME, SCHEMA_VERSION, EVENT_ID, XML)
                    .execute();
        } else {
    ...
    public void importCollectionData(DSLContext context)
        ...
        if (!Objects.isNull(schema)) {
            ...
            context.loadInto(COLLECTION_TABLE).onDuplicateKeyUpdate().commitAfter(100)
                    .loadCSV(inputStream).fields(UUID, NAME, VERSION, XML).execute();
        } else {
            throw new ObjectNotFoundException("No Schema found");
        }
    }
    ...
    public void importPropertyData(DSLContext context) throws IOException, ObjectNotFoundException {
        ...
        if (!Objects.isNull(schema)) {
            ...
            context.loadInto(ITEM_PROPERTY_TABLE).onDuplicateKeyUpdate().commitAfter(100)
                    .loadCSV(inputStream).fields(UUID, NAME, VALUE, MUTABLE).execute();
        } else {
            throw new ObjectNotFoundException("No Schema found");
        }
    }
    ...
    public void importJobData(DSLContext context) throws IOException, ObjectNotFoundException {
        ...
        if (!Objects.isNull(schema)) {
            ...
            context.loadInto(JOB_TABLE).onDuplicateKeyUpdate().commitAfter(100).loadCSV(inputStream)
                    .fields(UUID, ID, DELEGATE_UUID, ITEM_UUID, STEP_NAME, STEP_PATH, STEP_TYPE,
                            TRANSITION, ORIGIN_STATE_NAME, TARGET_STATE_NAME, AGENT_ROLE,
                            ACT_PROPERTIES, CREATION_TS)
                    .execute();
        } else {
            throw new ObjectNotFoundException("No Schema found");
        }
    ...
    public void importLifeCycleData(DSLContext context)
            throws IOException, ObjectNotFoundException {
        ...
        if (!Objects.isNull(schema)) {
            ...
            context.loadInto(LIFECYCLE_TABLE).onDuplicateKeyUpdate().commitAfter(100)
                    .loadCSV(inputStream).fields(UUID, NAME, XML).execute();
        } else {
            throw new ObjectNotFoundException("No Schema found");
        }
    }

    public void importAttachmentData(DSLContext context)
            throws IOException, ObjectNotFoundException {
        ...
        if (!Objects.isNull(schema)) {
            ...
            context.loadInto(OUTCOME_ATTACHMENT_TABLE).onDuplicateKeyUpdate().commitAfter(100)
                    .loadCSV(inputStream)
                    .fields(UUID, SCHEMA_NAME, SCHEMA_VERSION, EVENT_ID, ATTACHMENT).execute();
        } else {
            throw new ObjectNotFoundException("No Schema found");
        }
    }

    public void importViewPointData(DSLContext context)
            throws IOException, ObjectNotFoundException {
        ...
        if (!Objects.isNull(schema)) {
            ...
            context.loadInto(VIEWPOINT_TABLE).onDuplicateKeyUpdate().commitAfter(100)
                    .loadCSV(inputStream).fields(UUID, SCHEMA_NAME, NAME, SCHEMA_VERSION, EVENT_ID)
                    .execute();
        } else {
            throw new ObjectNotFoundException("No Schema found");
        }
    ...
    public void importHistoryData(DSLContext context) throws IOException, ObjectNotFoundException {
        ...
        if (!Objects.isNull(schema)) {
            ...
            final Field<Integer> STATEMACHINE_VERSION =
                    field(name("STATEMACHINE_VERSION"), Integer.class);
            final Field<String> STEP_NAME = field(name("STEP_NAME"), String.class);
            final Field<String> STEP_PATH = field(name("STEP_PATH"), String.class);
            final Field<String> STEP_TYPE = field(name("STEP_TYPE"), String.class);
            ...
            context.loadInto(EVENT_TABLE).onDuplicateKeyUpdate().commitAfter(100)
                    .loadCSV(inputStream)
                    .fields(UUID, ID, AGENT_UUID, DELEGATE_UUID, AGENT_ROLE, SCHEMA_NAME,
                            SCHEMA_VERSION, STATEMACHINE_NAME, STATEMACHINE_VERSION, STEP_NAME,
                            STEP_PATH, STEP_TYPE, ORIGIN_STATE_ID, TARGET_STATE_ID, TRANSITION_ID,
                            VIEW_NAME, TIMESTAMP)
                    .execute();
        } else {
            throw new ObjectNotFoundException("No Schema found");
    ...
    public void importItem(DSLContext context) throws IOException, ObjectNotFoundException {
        ...
        if (!Objects.isNull(schema)) {
            ...
            final Field<Boolean> IS_PASSWORD_TEMPORARY =
                    field(name("IS_PASSWORD_TEMPORARY"), Boolean.class);


            InputStream inputStream = new FileInputStream(file);

            context.loadInto(ITEM_TABLE).onDuplicateKeyUpdate().commitAfter(100)
                    .loadCSV(inputStream)
                    .fields(UUID, IOR, IS_AGENT, IS_PASSWORD_TEMPORARY, PASSWORD).execute();
        } else {
            throw new ObjectNotFoundException("No Schema found");
        }
    }
    ...
    public Schema getPublicSchema(DSLContext context) {
        return context.meta().getSchemas().stream().filter(s -> s.getName().equals("public"))
                .findFirst().get();
    }
}
