/**
 * This file is part of the CRISTAL-iSE XPath Outcome Initiator module. Copyright (c) 2001-2016 The
 * CRISTAL Consortium. All rights reserved.
 *
 ...
 * out even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU
...
import static org.cristalise.kernel.persistency.outcomeinit.SystemProperties.SimpleType_DefaultValues;

/*
 * Copied from project org.apache.xmlbeans version 3.0.1
 *
 * TODO: Comment on enumerations? Comment on facets? Have a verbose option? Have a sample data
 * option, would create valid instance with sample data? Add the pattern facet; this is tricky,
 * considering the relationship with length
 */
...
public class SampleXmlUtil {
    ...
    private SampleXmlUtil(boolean soapEnc) {
        _soapEnc = soapEnc;
        String propertyValues = SimpleType_DefaultValues.getString("");
    ...
    public static final String[] WORDS = new String[] {"ipsa", "iovis", "rapidum", "iaculata", "e",
            "nubibus", "ignem", "disiecitque", "rates", "evertitque", "aequora", "ventis", "illum",
            ...
            "soror", "et", "coniunx", "una", "cum", "gente", "tot", "annos", "bella", "gero", "et",
            "quisquam", "numen", "iunonis", "adorat", "praeterea", "aut", "supplex", "aris",
            "imponet", "honorem", "talia", "flammato", "secum", "dea", "corde", "volutans",
            "nimborum", "in", "patriam", "loca", "feta", "furentibus", "austris", "aeoliam",
            "venit", "hic", "vasto", "rex", "aeolus", "antro", "luctantis", "ventos",
            "tempestatesque", "sonoras", "imperio", "premit", "ac", "vinclis", "et", "carcere",
            "frenat", "illi", "indignantes", "magno", "cum", "murmure", "montis", "circum",
            "claustra", "fremunt", "celsa", "sedet", "aeolus", "arce", "sceptra", "tenens",
            "mollitque", "animos", "et", "temperat", "iras", "ni", "faciat", "maria", "ac",
            "terras", "caelumque", "profundum", "quippe", "ferant", "rapidi", "secum", "verrantque",
            "per", "auras", "sed", "pater", "omnipotens", "speluncis", "abdidit", "atris", "hoc",
            "metuens", "molemque", "et", "montis", "insuper", "altos", "imposuit", "regemque",
            "dedit", "qui", "foedere", "certo", "et", "premere", "et", "laxas", "sciret", "dare",
            "iussus", "habenas",};



    private static final String[] DNS1 =
            new String[] {"corp", "your", "my", "sample", "company", "test", "any"};
    private static final String[] DNS2 =
            new String[] {"com", "org", "com", "gov", "org", "com", "org", "com", "edu"};
...
//    }
    ...
    /**
     * Formats a given string to the required length, using the following operations: - append the
     * source string to itself as necessary to pass the minLength; - truncate the result of previous
     * step, if necessary, to keep it within minLength.
     */
...
//        BigInteger min, max;
    ...
    private String formatDate(SchemaType sType) {
        GDateBuilder gdateb = new GDateBuilder(new Date(
    ...
    private int determineMinMaxForSample(SchemaParticle sp, XmlCursor xmlc) {
        ...
        if (sp.getMaxOccurs() == null) {
            // xmlc.insertComment("The next " + getItemNameOrType(sp, xmlc) + " may be repeated " +
            // minOccurs + " or more times");
            if (minOccurs == 0)
    ...
    /*
     * Return a name for the element or the particle type to use in the comment for minoccurs, max
     * occurs
     */
...
//    }
    ...
    private static final QName ENC_ARRAYTYPE =
            new QName("http://schemas.xmlsoap.org/soap/encoding/", "arrayType");
    private static final QName ENC_OFFSET =
            new QName("http://schemas.xmlsoap.org/soap/encoding/", "offset");
    ...
    private void processAttributes(SchemaType stype, XmlCursor xmlc) {
        ...
        for (int i = 0; i < attrProps.length; i++) {
            ...
            xmlc.insertAttributeWithValue(attr.getName(),
                    defaultValue == null ? sampleDataForSimpleType(attr.getType()) : defaultValue);
        }
    }
    ...
    private void processChoice(SchemaParticle sp, XmlCursor xmlc, boolean mixed) {
        SchemaParticle[] spc = sp.getParticleChildren();
        xmlc.insertComment("You have a CHOICE of the next " + String.valueOf(spc.length)
                + " items at this level");
        for (int i = 0; i < spc.length; i++) {
            processParticle(spc[i], xmlc, mixed);
        }
    ...
    private void processAll(SchemaParticle sp, XmlCursor xmlc, boolean mixed) {
        SchemaParticle[] spc = sp.getParticleChildren();
        // xmlc.insertComment("You may enter the following " + String.valueOf(spc.length) + " items
        // in any order");
        for (int i = 0; i < spc.length; i++) {
            processParticle(spc[i], xmlc, mixed);
            if (mixed && i < spc.length - 1)
...
//    }
    ...
    private List<SchemaType> _typeStack = new ArrayList<>();
