...
CRISTAL-iSE is based on three major concepts to define the possible scope of change: **relationship**, **process**, and **data**.

* **Relationship**: The scope of change is defined by **Collections**. Relationships between Items are not hard-coded but are managed as separate, versionable descriptions. This allows for dynamic restructuring of the domain model and its relationships without modifying the Items themselves.
* **Process**: The scope of change is defined by **LifeCycles**. Business logic is encapsulated in versionable LifeCycle descriptions, which can be evolved and versioned independently. This means that different Items can follow different versions of a process, supporting gradual transition and experimentation.
* **Data**: The scope of change is defined by **Outcomes** and their associated **Schemas**. Data produced during the execution of a process is stored as XML documents alongside with their version  descriptions, i.e. Schema. This enables the evolution of data models alongside the processes that produce them, ensuring that historical data remains valid while new data follows new structures.

By using these concepts, CRISTAL-iSE allows developers to define the scope of change at a very granular level, making it possible to evolve a system incrementally and safely over a long period of time. Identifying the scope of change also enables the system to use and implement various migration techniques and tools.
